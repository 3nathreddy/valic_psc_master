...
## START: vd-devops-mod, place first dispatcher config

cookbook_file "#{node[:apache][:dir]}/conf/aem-farms/farm_1.any" do
  source 'farm_1.any'
  owner 'root'
  group node[:apache][:root_group]
  mode '0664'
...
# directory
directory "/opt/aem/dispatcher" do
  owner node[:apache][:user]
  group node[:apache][:root_group]
  mode '0775'
  recursive true
...
## END: vd-devops-mod, place first dispatcher config

## START: vd-devops-mod, httpd conf

cookbook_file "#{node[:apache][:dir]}/conf/httpd.conf" do
  source 'httpd.conf'
  owner 'root'
  group node[:apache][:root_group]
  mode '0664'
...
## START: vd-devops-mod, disable iptables in local build
...
## END: vd-devops-mod, disable iptables in local build
